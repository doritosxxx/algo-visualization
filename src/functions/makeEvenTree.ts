...
function unfold<T extends character>(tree: SuffixTreeNode<number>, indexes: IndexedPair<T>[]): SuffixTreeNode<T> {
    const unfolded = new SuffixTreeNode<T>();
    for (const edge of tree.edges) {
        const word = edge.substring.map((letter) => indexes[letter]).flatMap((pair) => [pair.first, pair.second]);

        const unfolderEdge = new SuffixTreeEdge<T>(word);
        unfolderEdge.endNode = unfold(edge.endNode, indexes);
...
export default function makeEvenTree<T extends character>(
    ...
    indexes: IndexedPair<T>[]
): SuffixTreeNode<T> {
    const evenTree = unfold(tree, indexes);
